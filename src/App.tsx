...
export default function App() {
  ...
  const [activeSessionId, setActiveSessionId] = useState<string>("session-mira-102");
  ...
  const [aiBriefing, setAiBriefing] = useState<AISummary>({
    currentProject: "Mira VPN Client",
    focus: "Review authentication payload matching JWT signature variables",
    completed: [
      "Docker setup initialized for vpn-auth channel",
      "Drafted authentication middleware configuration structure"
    ],
    pending: [
      "Resolve connection timeout with the VPN test socket",
      "Commit your 2 stashed changes in auth branch before turn ends",
      "Deploy authentication script validation"
    ],
    risks: [
      "No git commits in 4 hours. 2 changes are fully stashed.",
      "Clipboard currently contains a secret production key copied 8 minutes ago & never pasted!"
    ]
  });
  ...
  const [notifications, setNotifications] = useState<Array<{ id: string; title: string; text: string; type: 'warning' | 'info' | 'clipboard' }>>([
    {
      id: "notif-1",
      title: "Context Lost Warning",
      text: "You switched between project structures 7 times in the last hour. Deep breathing recommended.",
      type: "warning"
    },
    {
      id: "notif-2",
  ...
  // Simulated active session reference from timeline
  ...
  const generateAIConsistentSummary = async () => {
    ...
    try {
      const res = await fetch("/api/gemini/summarize", {
        ...
        body: JSON.stringify({
          currentState,
          timelineLog: activeSession.timeline,
          customResumeTask
        })
      });
            ...
            <div className="space-y-4">
              ...
              {errorMessage && (
                <p className="text-[10px] text-yellow-500 hover:underline text-center cursor-pointer font-mono pt-1">
                  Heuristic active format output parsed. Configure GEMINI_API_KEY to test actual LLM summaries.
                </p>
              )}
